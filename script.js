...
function sortByName(sortInfo, sortDirect) {
  ...
  if (sortDirect == "asc") {
    ...
    sortedList = allStudents.sort(compareDesc);
...
function preapareObject(jsonObject) {
  ...
  if (studentName.indexOf(" ") === -1) {
    ...
    student.lastname = studentName.substring(studentName.lastIndexOf(" ") + 1, studentName.length);
    student.lastname = student.lastname[0].toUpperCase() + student.lastname.substring(1, student.lastname.length).toLowerCase();

    if (student.lastname.indexOf("-") !== -1) {
      student.lastname = student.lastname.substring(0, student.lastname.indexOf("-") + 1) + student.lastname[student.lastname.indexOf("-") + 1].toUpperCase() + student.lastname.substring(student.lastname.indexOf("-") + 2, student.lastname.length);
    }
  }

  //capitalie first name
  student.firstname = student.firstname[0].toUpperCase() + student.firstname.substring(1, student.firstname.length).toLowerCase();
  ...
  if (studentName.indexOf(" ") !== studentName.lastIndexOf(" ")) {
    ...
    if (studentName.indexOf(" ") + 1 == studentName.indexOf('"')) {
      student.nickname = studentName.substring(studentName.indexOf('"') + 1, studentName.lastIndexOf('"'));
      student.nickname = student.nickname[0].toUpperCase() + student.nickname.substring(1, student.nickname.length).toLowerCase();
    }
    // Find only middlename and capitalize
    else {
      student.middlename = studentName.substring(studentName.indexOf(" ") + 1, studentName.lastIndexOf(" "));
      student.middlename = student.middlename[0].toUpperCase() + student.middlename.substring(1, student.middlename.length).toLowerCase();
...
function displayStudent(student) {
  ...
  clone.querySelector(".student_img").addEventListener("click", modal);
  ...
  function togglePrefect(thisStudent) {
    const totalPrefects = allStudents.filter((student) => (student.prefect === "true" ? true : false));
    const sameTypeHouses = totalPrefects.filter((student) => (student.house === thisStudent.house ? true : false));
    console.log(sameTypeHouses.length);

    if (thisStudent.prefect === "true") {
      thisStudent.prefect = "false";
      displayList(allStudents);
    } else if (totalPrefects.some((prefect) => prefect.house === thisStudent.house)) {
  ...
  function callAlertMoreThan2(prefects, newPrefect) {
    ...
    for (let i = 0; i < 2; i++) {
      document.querySelector(`.student${1 + i}`).textContent = `${prefects[i].firstname}, ${prefects[i].house}`;
    }
  ...
  function callAlertSameHouse(sameHouse, newPrefect) {
    ...
    document.querySelector("#onlyonehouse .student1").textContent = `${sameHouse.firstname},  ${sameHouse.house}`;
    document.querySelector("#onlyonehouse [data-action=remove1]").addEventListener("click", removeCurrentPrefect);
    document.querySelector("#onlyonehouse.closebutton").addEventListener("click", keepCurrentPrefect);
    function removeCurrentPrefect() {
      ...
      document.querySelector("#onlyonehouse[data-action=remove1]").removeEventListener("click", removeCurrentPrefect);
      document.querySelector("#onlyonehouse .closebutton").removeEventListener("click", keepCurrentPrefect);
    }
  ...
  function removeButton(prefects, newPrefect) {
    const removeBtnArray = [document.querySelector(`[data-action=remove1`), document.querySelector(`[data-action=remove2`)];
  ...
  function modal() {
    ...
    // modal.querySelector(
    //   ".modal-img"
    // ).src = `images/${student.firstname.toLowerCase()}_.png`;
    if (student.lastname.includes("-")) {
      modal.querySelector(".student_img").src = `images/${student.lastname.substring(student.lastname.indexOf("-") + 1)}_${student.firstname[0]}.png`;
    } else {
      modal.querySelector(".modal_img").src = `images/${student.lastname}_${student.firstname[0]}.png`;
    }
    if (student.lastname.includes("Patil")) {
      modal.querySelector(".modal_img").src = `images/${student.lastname}_${student.firstname[0]}.png`;
    }
    ...
    modal.querySelector("object").setAttribute("data", "images/" + student.house + ".png");
    console.log("images/" + student.house + ".png");
    modal.querySelector("[data-field=squad]").addEventListener("click", clickSquad);
